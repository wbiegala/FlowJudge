...
import { Authenticate, ClearTenantContext } from './store/tenant.actions';
...
export const accessTokenInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  const currentToken = store.selectSnapshot(TenantState.accessToken);
      ...
      if (err.status === 401 && !isRefreshEndpoint(req.url)) {
        ...
        if (!refreshInFlight$) {
          refreshInFlight$ = authService.refreshToken().pipe(
            ...
            map(() => store.selectSnapshot(TenantState.accessToken) ?? ''),
