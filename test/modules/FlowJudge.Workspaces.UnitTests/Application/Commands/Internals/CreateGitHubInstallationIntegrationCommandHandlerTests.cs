...
{
    public class CreateGitHubInstallationIntegrationCommandHandlerTests
    {
        [Fact]
        public async Task ExecuteAsync_WhenIssuerHasPermissions_ThenCreateInactiveGitHubInstallationIntegration()
        {
            // Arrange
            IntegrationRoot? addedIntegration = null;
            ...
            _integrationRepositoryMock
                .Setup(ir => ir.AddIntegrationAsync(It.IsAny<IntegrationRoot>(), It.IsAny<CancellationToken>()))
                .Callback<IntegrationRoot, CancellationToken>((savedIntegration, _) => addedIntegration = savedIntegration)
                .Returns(Task.CompletedTask);

            var command = Fixture.CreateGitHubInstallationIntegrationCommand(
                WorkspaceId,
                IssuerId,
                IntegrationName,
            ...
            Assert.Same(integration, addedIntegration);
            Assert.Equal(IntegrationStatus.Inactive, addedIntegration!.Status);
            var authentication = Assert.Single(addedIntegration.AuthenticationData);
            Assert.Equal(IntegrationAuthenticationType.InstallationId, authentication.Type);
            Assert.Equal(IntegrationAuthenticationStatus.Active, authentication.Status);
            Assert.Equal(GitHubInstallationId, authentication.Value.Value);
            Assert.Equal(CreationTimestamp, authentication.CreatedAt);
            Assert.Equal(IssuerId, authentication.CreatedBy);
        ...
        public async Task ExecuteAsync_WhenIssuerHasInsufficientPermissions_ThenReturnFailure()
        {
            ...
            _integrationRepositoryMock.Verify(ir => ir.AddIntegrationAsync(
        ...
        private CreateGitHubInstallationIntegrationCommandHandler CreateHandler() =>
            new(
